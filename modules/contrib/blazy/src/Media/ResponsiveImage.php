...
class ResponsiveImage {
   ...
   *
   * @return \Drupal\breakpoint\BreakpointManagerInterface|null
   *   The breakpoint manager or null.
   */
  public static function breakpointManager(): ?BreakpointManagerInterface {
  ...
  public static function background(array &$attributes, array &$settings): void {
    ...
    if ($styles = self::styles($resimage)) {
      ...
      foreach (array_values($styles['styles']) as $style) {
        $dims = Image::transformDimensions($style, $blazies);
        $width = $dims['width'];
        ...
        $sets = $dims + $settings;
        $data = Image::background($sets, $style);
        $srcset[$width] = $data;
   ...
   *
   * @todo enable at 4.x \Drupal\responsive_image\ResponsiveImageStyleInterface|null $resimage.
   */
  public static function dimensions(
    ...
    foreach (array_values($styles['styles']) as $style) {
      ...
      // @fixme $initial.
      $data = Image::transformDimensions($style, $blazies);
      $width = $data['width'];
   ...
   *
   * @param \Drupal\responsive_image\ResponsiveImageStyleInterface $resimage
   *   The responsive image style entity.
   *
   * @return array
   *   The responsive image styles and cache tags.
   *
   * @todo enable at 4.x \Drupal\responsive_image\ResponsiveImageStyleInterface $resimage.
   */
   ...
   * @todo move it out into a DI class at D11.
   */
  public static function fallback(array &$settings, string $placeholder): void {
    $manager = Internals::blazy();
    if (!$manager) {
      return;
    }
    ...
    if ($resimage = $blazies->get('resimage.style')) {
      ...
      if ($fallback == $id) {
        ...
        if ($uri && $style = $manager->load($id, 'image_style')) {
          $data_src = Url::fromAny($settings, $style, $uri);
          ...
          [
            ...
            'height' => $height,
            'ratio'  => $ratio,
          ] = Image::transformDimensions($style, $blazies, $tn_uri);
   ...
   *   The given settings.
   * @param bool $unstyled
   *   Whether image which should not have image styles like SVG, etc.
   *
   * @return \Drupal\responsive_image\ResponsiveImageStyleInterface|null
   *   The responsive image style.
   */
  public static function toStyle(
    array $settings,
    bool $unstyled = FALSE,
  ...
  private static function define(
    ...
    ResponsiveImageStyleInterface $resimage,
  ): void {
    $id = $resimage->id();
    $styles = self::styles($resimage);
   ...
   * @param array $settings
   *   The modified settings.
   * @param \Drupal\responsive_image\ResponsiveImageStyleInterface|null $style
   *   The responsive image style.
   *
   * @return array
